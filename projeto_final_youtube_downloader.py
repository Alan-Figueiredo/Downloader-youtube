...
while True:
    ...
    def mp3(progresso_mp3):
        ...
        path = Path(diretorio)       
        lista = [filename1 for filename1 in list(path.glob('*')) if filename1.suffix =='.webm' ] # Logica para procurar pela extensão.
        ...
        if escolha == '1':
            ...
            for links in progresso_playlist_mp3.video_urls:
                try:
                    ...
                    stream_download = stream.streams.get_by_itag(251)
                    stream_download.download(diretorio)

                    print(f'{stream_download.title} Download completo.')
                except:
            ...
            for links in progresso_playlist_mp4.video_urls:
                ...
                stream = YouTube(links,on_progress_callback=on_progress)
                stream.streams.filter(progressive=True)
